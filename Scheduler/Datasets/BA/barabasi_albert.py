#! /usr/bin/python
'''
Generates a Barabasi Albert graph and creates a random set of flows with period
Output - 
    links.dat -> (n1, n2)
    flows.dat -> (src, dst, p)
    readme
...
randGraph = nx.barabasi_albert_graph(nSwitches, nDegree).to_directed()
random.seed(nSeed)
...
strDir = "BA-s"+str(nSwitches)+"-h"+str(nHosts)+"-d"+str(nDegree)+"-f"+str(nFlows)+"-mp"+str(maxPeriod)+"-md"+str(maxDst)+"-seed"+str(nSeed)+"-c"+str(coreFlowsOnly)
try:
    os.mkdir(strDir)
except OSError:
    pass    
